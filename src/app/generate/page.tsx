...
function ColorColumn({
  ...
  return (
    <motion.div
      layout
      className="relative flex-1 flex flex-col items-center justify-center cursor-pointer select-none min-h-[200px] md:min-h-0 rounded-2xl overflow-hidden"
    ...
    >
      ...
      <AnimatePresence>
        {locked && (
          <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }}
            className="absolute top-5 left-1/2 -translate-x-1/2" style={{ color: textColor }}>
            <div className="p-2 rounded-full" style={{ backgroundColor: btnBg }}><IconLock size={13} /></div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="text-center z-10 px-4">
        <motion.p key={color} initial={{ opacity: 0.7, y: -2 }} animate={{ opacity: 1, y: 0 }}
          className="font-mono text-2xl sm:text-3xl md:text-4xl font-bold tracking-wider mb-1.5"
          style={{ color: textColor }}>
          {color.toUpperCase()}
        </motion.p>
        <p className="text-[13px] font-medium mb-3 tracking-wide" style={{ color: textMuted }}>{name}</p>
        ...
        <div className="flex items-center gap-1.5 justify-center">
          {([
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg text-[10px] font-mono font-semibold transition-all duration-150 hover:scale-105 active:scale-95"
              style={{
      ...
      {/* Action buttons */}
      <motion.div initial={false} animate={{ opacity: hovered ? 1 : 0, y: hovered ? 0 : 10 }}
        ...
        {([
          ...
          <button key={btn.label} onClick={(e) => { e.stopPropagation(); btn.action(); }}
            className="p-3 rounded-2xl transition-all duration-150 hover:scale-110 active:scale-95"
      ...
      <AnimatePresence>
        {showShades && (
          <motion.div initial={{ opacity: 0, y: -8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
            ...
            className="absolute bottom-24 left-1/2 -translate-x-1/2 flex rounded-2xl overflow-hidden shadow-2xl z-20"
            ...
            {shades.map((s, i) => (
              <button key={i} onClick={(e) => { e.stopPropagation(); onSetColor(s); setShowShades(false); }}
                className="w-9 h-12 md:w-11 md:h-16 hover:scale-y-110 transition-transform duration-150 relative group/shade"
    ...
    <div className="h-screen flex flex-col pt-14">
      ...
      <div className="glass border-t border-[var(--color-border)] px-4 sm:px-6 py-3">
        <div className="flex items-center justify-center gap-2 sm:gap-3 flex-wrap max-w-[1200px] mx-auto">
          ...
          <div className="hidden sm:block w-px h-6 bg-overlay-6" />
      ...
      {/* Cross-links */}
      <div className="mt-12 mb-6 px-4 max-w-4xl mx-auto">
